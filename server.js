...
// Book.find().sort({ author: 'asc' }).exec(function (err, book) {
//   for (var i = 0; i < book.length; i++) {
//     if (book[i].rating  < 5)
//       console.log(book[i].author + ' '+book[i].title + book[i].rating)
//   }
// });

//3. Find all the Fiction books, skip the first 2, and display only 3 of them 
// Book.find({genres: 'Fiction' }).skip(2). limit(3).exec(function(err, book) {
//   for(var i = 0;  i < book.length; i++){
//   console.log (book[i].title +' '+ book[i].genres) }});
...
// Person.find( { $or:[ {height:{"$gt":180}}, {salary:{"$gt":30000}} ]}).exec(function(err,person){
//   for (var i = 0; i < person.length; i++){
//   console.log(person[i].height +' '+person[i].salary)}});

//3. Find all the people who have grey hair or eyes, and are skinny (<70)
// Person.find({ $and: [{ hair: 'grey'}, {weight:{"$lt":70}}] }).exec(function(err,person){
//     for (var i = 0; i < person.length; i++){
//     console.log(person[i].hair +' '+person[i].weight)}});


//4. Find people who have at least 1 kid with grey hair
// Person.find({kids:{$elemMatch:{hair:'grey'}}}).exec(function(err,person){
//     for (var i = 0; i < person.length; i++){
//     console.log(person[i].kids)}});

// //5. Find all the people who have at least one overweight kid, and are overweight themselves (>100)
// Person.find({ $and: [{ weight:{"$gt":100}}, {kids:{$elemMatch:{weight:{"$gt":100}}}}]}).exec(function(err,person){
//     for (var i = 0; i < person.length; i++){
//     console.log(person[i].kids +' '+person[i].weight)}});